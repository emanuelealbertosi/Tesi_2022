...
def main():
    ...
    file = open("/Users/macintosh/Documents/tesi/funzioni/deployment_order.yaml", "w")
                                ...
                                if links['btype']=='TRUE':
                                    for pname,nodename in placement.items():
                                        ...
                                        if ('ServicePad' in pname and nodename==placement[links['fun']] and not(pname in worked_pad) and truedone==0):
                                                ...
                                                #print("placement links fun= "+placement['frules'])
                                                #print("links fun= "+links['fun'])
                                                #pad_dep,pad_serv=creaPad(placement[links['fun']],pname.lower(),links['fun'])
                                                pad_dep,pad_serv=creaPad(placement['frules'],pname.lower(),links['fun'])
                                    ...
                                    for pname,nodename in placement.items():
                                        ...
                                        if ('ServicePad' in pname and nodename==placement[links['fun']] and not(pname in worked_pad) and falsedone==0):
                                                #cont_details['env'].append({'name':str(links['name']),'value':'http://'+pname.lower()+':5000'})
                                                cont_details['env'].append({'name':str(links['name']),'value':'http://frules:5000'})
                                                
                                                #print("placement links fun= "+placement['fcheckdcc'])
                                                #print("links fun= "+links['fun'])
                                                #pad_dep,pad_serv=creaPad(placement[links['fun']],pname.lower(),links['fun'])
                                                pad_dep,pad_serv=creaPad(placement['fcheckdcc'],pname.lower(),links['fun'])
                                                deployment['items'].append(pad_dep)
                                                deployment['items'].append(pad_serv)
                                                worked_pad.append(pname)
                                                falsedone=1
...
def creaPad(node,fun_name,link_fun):
            ...
            cont_details['ports']=[]
            cont_details['env']=[]
            #if cablato, per renderlo piu' generale bisognerebbe creare un file di configurazione
            if (link_fun=='frules'):
                cont_details['image']="albertosiemanuele/tesitestv1:fpadfrulesorder"
            else:
                cont_details['image']="albertosiemanuele/tesitestv1:fpadfcheckdcc"
...
def outPutPlace(placement,application,infrastructure):
    ...
    for fun in ['fServicePad1','fServicePad2']:
